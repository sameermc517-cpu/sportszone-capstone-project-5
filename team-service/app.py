...
def _init_db(retries: int = 10, delay: int = 3) -> None:
    """Retry db.create_all() so the service survives a slow Postgres start.
    Kubernetes will crash-restart the pod if this ultimately fails, but the
    retry loop avoids unnecessary restarts when Postgres is just a few seconds
    behind the app container."""
